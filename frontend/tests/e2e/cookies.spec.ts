...
test.describe('Cookie Banner', () => {
  ...
  test('banner is visible on first visit', async ({ page }) => {
    await page.goto('/')
    await expect(
      page.getByRole('banner', { name: 'Cookie consent' }),
    ).toBeVisible()
  })
  ...
  test('returning to / after visiting cookie policy still shows banner', async ({
    page,
  }) => {
  ...
  test('clicking "Agree" hides banner and it does not reappear on next visit', async ({
    page,
  }) => {
    await page.goto('/')
  ...
  test('outdated stored consent causes banner to reappear', async ({
    context,
    page,
  }) => {
